...
GRAPHDRONE_VERSION = "v1-width.2026.03.15"  # bump when model changes to invalidate cache
...
def run_graphdrone(X_tr, y_tr, X_te, task_type: str, seed: int = 42, n_classes: int = None):
    # v1-width API: problem_type goes into fit(), not GraphDroneConfig.
    # Router kind is "noise_gate_router" (ContextualTransformerRouter with GORA observers).
    # No CatBoost/XGBoost — foundation specialists only.
    # Classification only supports "binary"; multiclass raises an error.
    n = X_tr.shape[1]
    ...
    if task_type == "regression":
        ...
        # v1-width only supports binary classification
        if n_classes is None:
            n_classes = int(len(np.unique(y_tr)))
        if n_classes > 2:
            raise ValueError(f"v1-width only supports binary classification; got n_classes={n_classes}")
        fit_problem_type = "binary"
        specs = (
            ExpertBuildSpec(
                descriptor=ViewDescriptor(
                    expert_id="FULL", family="FULL", view_name="Foundation Full",
    ...
    # v1-width: no problem_type/n_classes in GraphDroneConfig; use noise_gate_router
    cfg = GraphDroneConfig(router=SetRouterConfig(kind="noise_gate_router"))
    gd = GraphDrone(cfg)
    gd.fit(X_tr, y_tr, expert_specs=specs, problem_type=fit_problem_type)
    ...
    if task_type == "regression":
        ...
        # Binary: preds is [N] probability of class 1
