...
## Current best ELO (2026-03-23, v1.3.0-rc) — **BOTH ENGINES WIN**

| Engine | GD ELO | TabPFN ELO | Benchmark | Tasks |
|---|---|---|---|---|
| **Regression** | **1523.2** | 1476.8 | geopoe, v1-geopoe-2026.03.19c | 36/36 |
| **Classification** | **1507.9** | 1492.1 | smart benchmark, 2026.03.23-clf-v1.3-phase2 | 54/54 |

Both engines are in `main`. One `GraphDrone` class dispatches via `_detect_problem_type(y)`.

### Classification per-dataset (v1.3-phase2, smart benchmark — 9 datasets × 3 folds)
| Dataset | GD F1 | TPF F1 | GD log_loss | TPF log_loss | Result |
|---|---|---|---|---|---|
| diabetes (binary) | 0.7405 | 0.7320 | 0.4738 | 0.4736 | **GD wins F1 +0.009** |
| credit_g (binary) | 0.6923 | 0.6937 | 0.4805 | 0.4760 | Gap: −0.0014 F1 (was −0.004, 65% closed) |
...
- Current classification version: `2026.03.23-clf-v1.3-phase2`
...
| **2026-03-23** | **v1.3.0-rc** | **1523.2** | **1507.9** | **← current branch**. TaskConditionedPrior + confidence-gated defer penalty. credit_g gap −0.004→−0.0014. |

---

## Known gaps (future work)

1. **credit_g still lags TabPFN** (−0.0014 F1 after Phase 2, was −0.004). Task-prior narrowed the gap 65%. Remaining gap: score ordering at the threshold level (Phase 3). Next: expert-allocation ordering loss (3A) or dataset-conditioned threshold model (3B).
