...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TabM on OpenML classification task splits")
...
def main() -> None:
    ...
    resolved_config = resolve_tabm_config_name(split) if args.config == "auto" else args.config
    source_config = args.upstream_root / "exp" / f"{resolved_config}.toml"
    ...
    prediction_type = str(upstream_report.get("prediction_type", "probs"))
        ...
        "rows": [
            {
                ...
                "notes": "metrics recomputed from upstream TabM predictions.npz",
            }
        ],
